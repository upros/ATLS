...
they leverage {{?RFC5705}} for exporting keys. Hence, when the OSCORE mode has 
...
packet protection. These keys are produced using a TLS exporter {{?RFC5705}} 
to generate 2 x the number of bytes required by the established ciphersuite. 
For example, if a TLS/DTLS 1.2 handshake negotiated the TLS_PSK_WITH_AES_128_CCM_8 
ciphersuite then the key extractor is requested to produce 2 x 128 bit keying material. 
The per-association context value for the key extractor is empty.

The exporter label for this usage is "application-layer-tls". 

The following parameters are needed for use with OSCORE: 

 - Master Secret: The master secret corresponds to the first half
   of the keying material generated by the key exporter. 
 ...
 - Master Salt: The master salt corresponds to the second half 
   of the keying material generated by the key exporter. 

-  HKDF Algorithm: This value is negotiated using the ciphersuite exchange provided by 
   the TLS/DTLS handshake. As a default, SHA256 is assumed as a HKDF algorithm. 
